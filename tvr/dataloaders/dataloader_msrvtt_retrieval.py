# @title writefile dataloader_msrvtt_retrieval.py
...
class MSRVTTDataset(RetrievalDataset):
    ...
    def _get_anns(self, subset='train'):
        ...
        """
        csv_path = {'train': join(self.anno_path, 'MSRVTT_train.7k.csv'),
                    'val': join(self.anno_path, 'MSRVTT_JSFUSION_test.csv'),
                    'test': join(self.anno_path, 'MSRVTT_JSFUSION_test.csv'),
                    'train_test': join(self.anno_path, 'MSRVTT_train.7k.csv')}[subset]
        if exists(csv_path):
        ...
        if subset in ['train', 'train_test']:
            anno_path = join(self.anno_path, 'MSRVTT_data.json')
            data = json.load(open(anno_path, 'r'))
            used = set()
            for itm in data['sentences']:
                vid = itm['video_id']
                if vid in video_id_list and vid not in used:
                    video_fp = join(self.video_path, f"{vid}.mp4")
                    if exists(video_fp):
                        used.add(vid)
                        video_dict[vid] = video_fp
                        sentences_dict[len(sentences_dict)] = (vid, (itm['caption'], None, None))
        else:
            for _, itm in csv.iterrows():
                ...
                if exists(video_fp):
                    video_dict[vid] = video_fp
                    sentences_dict[len(sentences_dict)] = (vid, (itm['sentence'], None, None))

        unique_sentence = set([v[1][0] for v in sentences_dict.values()])
        print(f'[{subset}] Unique sentence is {len(unique_sentence)} , all num is {len(sentences_dict)}')
