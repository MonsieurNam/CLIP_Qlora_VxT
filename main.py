# @title writefile main.py for multiprocess DDP
...
def get_args(description='Temporal Token Merging for Efficient Text-Video Retrieval'):
    ...
    parser.add_argument("--local_rank", dest="local_rank", type=int, default=0, help="DDP local rank")
...
def set_seed_logger(args):
    ...
    # --- START: Recommended changes for DDP initialization ---
    if torch.cuda.is_available():
        # torchrun sẽ tự động đặt các biến môi trường này.
        # Lấy local_rank từ biến môi trường thay vì đối số dòng lệnh.
        args.local_rank = int(os.environ["LOCAL_RANK"])
        args.world_size = int(os.environ["WORLD_SIZE"])
        
        # Đặt thiết bị cho tiến trình hiện tại
        torch.cuda.set_device(args.local_rank)
        args.device = torch.device("cuda", args.local_rank)

        # Khởi tạo process group. 'env://' sẽ tự động đọc các biến môi trường.
        torch.distributed.init_process_group(backend="nccl", init_method="env://")
        
        # Đảm bảo tất cả các tiến trình đã được khởi tạo trước khi tiếp tục
        torch.distributed.barrier()
        
        # Lấy rank toàn cục (global rank) sau khi khởi tạo
        global_rank = torch.distributed.get_rank()
        logger.info(
            f"DDP Initialized: local_rank={args.local_rank}, global_rank={global_rank}, world_size={args.world_size}"
        )
    ...
    # Chỉ in các tham số ở tiến trình chính để tránh log bị lặp
    if is_main_process():
